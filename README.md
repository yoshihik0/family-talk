...
会話とアカウントはサーバー側に残っているので、消えることはありません。入れなくなるのは、その端末のログイン情報が失われたときだけです(機種変更、ブラウザのデータ消去、端末の初期化など)。

**家族のだれかが入れなくなった場合**は、管理者が設定画面から新しい接続リンクを出して渡してください。元の名前のまま戻れます。

### 管理者が端末を失ったとき

このアプリの管理者は、そのデプロイのCloudflareアカウントを持っている人です。パスワードや復旧コードのような、別に覚えておくものはありません。Cloudflareを操作できることが、そのまま管理者である証明になります。

アプリを開くと出る画面に、そのまま使えるAI用プロンプトが表示されます。設置したときと同じように、それをコピーしてAIエージェントに渡してください。手元で操作する場合は、次の3つだけ分かれば復旧できます。

1. Cloudflareアカウント
2. アプリのURL(例: `https://omiya.example.workers.dev`)
3. サーバー名 = URLの先頭部分(例: `omiya`)。`npx wrangler d1 list` でも確認できます

**セットアップしたディレクトリが残っている場合**

```bash
npm run recover -- --url https://<自分のURL>
```

**ディレクトリが残っていない場合**は、クローンし直して `--database` でサーバー名を渡します。`wrangler.toml` を書き換える必要はありません。
...
npm install
npx wrangler login
npm run recover -- --url https://<自分のURL> --database <サーバー名>
```

24時間だけ有効な、一度きりの接続リンクが表示されます。それをスマートフォンで開けば、元の名前のまま管理者として戻れます。家族のだれかの分を出したいときは `--member "名前"` を付けてください。

**リンクは1回しか使えません。** 動作確認のつもりで開くと、その時点で無効になり、本人が使えなくなります。AIエージェントに代行してもらう場合も、開かずにURLだけ受け取ってください。
