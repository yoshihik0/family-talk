...
import { identities, invites, spaceMembers, spaces } from '@/db/schema';
...
export async function POST(request: Request) {
  ...
  await db.insert(spaceMembers).values({
    ...
    spaceId: claimedInvite.spaceId,
    identityId,
    role: claimedInvite.role,
    capabilities: { records: ['read', 'create'] },
    createdAt: now,
  });
  ...
  const response = Response.json({ ok: true, spaceName: space?.name ?? '' });
